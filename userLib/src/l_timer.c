...
void TimingDelayUS_Decrement(void)
{
    ...
    for(int i = 0; i < MTABSIZE(g_ustimer); i++) {
        if(g_ustimer[i].tick_bak > 0) {
            if(g_ustimer[i].tick > 0) {
                g_ustimer[i].tick--;
            } else {
                g_ustimer[i].tick = g_ustimer[i].tick_bak;
                
                msg.msgType = g_ustimer[i].msgType;
                msg.msgValue = i;
                msgq_in(&g_msgq, &msg);
            }
        }
...
{
#define CUART1_TIMER_OUT (9)  // 暂没特殊意义，大于下来的3就行
    if (g_IT_uart1_tmr < CUART1_TIMER_OUT) {
        g_IT_uart1_tmr++;
        if (g_IT_uart1_tmr == 3) {
            msg_t msg;
            msg.msgType = CUART1_TOUT;
            msgq_in(&g_msgq, &msg);
        }
    }
}
...
{
/**
 * 0. CVOPTMR_INIT 作为定时器自增上限
 * 1. 0 - CVOPTMR_S 作为确保与上一条语音间隔的的时间(>5ms)；CVOPTMR_S时刻，先给一个复位动作(防止上电不确定状态乱动作)
 * 2. 
 **/
#define VOPRESET_L(x)
#define VOPRESET_H(x)
